#Code for ResNet50 with height and width 224
...
def get_siamese_model(input_shape, embedding_dim=128):
    ...
    net = Dense(embedding_dim, activation='relu', name='embed')(net)
    net = Dropout(0.5)(net)
    net = Dense(embedding_dim, activation='relu', name='embed2')(net)
    net = Dropout(0.5)(net)
    net = Dense(embedding_dim, activation='relu', name='embed3')(net)
...
#Fit the model
model = get_siamese_model((224,224,3),  embedding_dim=128)
model.summary()
...
model.save("/content/drive/My Drive/IML Project 4 Data/resnet224_model.h5")
...
model = load_model("/content/drive/My Drive/IML Project 4 Data/resnet224_model.h5", compile = False)  
...
for i in range(batch_size):
    ...
    anchor =  np.reshape(np.array(anchor, dtype = "float"), (1 , 224, 224, 3))
    positive = np.reshape(np.array(positive, dtype = "float"), (1, 224, 224,3))
    negative =  np.reshape(np.array(negative, dtype = "float"), (1, 224, 224,3))
...
#Save predictions
np.savetxt("resnet224_PREDS.txt",pred, fmt="%d")